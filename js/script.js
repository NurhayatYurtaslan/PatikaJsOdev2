...
for (let i = 0; i < close.length; i++) {
  // class'ı "close" olan her item için
...
function yeniElement() {
  ...
  if (inputValue === "" || inputValue.replace(/^\s+|\s+$/g, "").length == 0) {
    // eğer değer girilmeden ekleye tıklanmışsa veya değer trim yapıldıktan sonra boş string kalıyorsa
...
input.addEventListener("keyup", function (event) {
  // belirtilecek tuşa basınca fonksiyon çalışsın
  if (event.key === "Enter") {
    // eğer tuş "Enter" ise
...
listYaniUL.addEventListener(
  ...
  function (event) {
    // addEventListener ikinci parametre çalışacak fonksiyon
    if (event.target.tagName === "LI") {
      // eğer tıklanan şey "li" ise
